	...
	server.post(
		...
		async (req: FastifyRequest<{ Body: ConvertBody }>, res: FastifyReply) => {
            ...
            if (collectiblesToConvert.length != 10) {
                return res.code(400).send({ error: 'You need 10 collectibles to convert to a higher rarity' })
            }

            // check for ownership
            for (const c of collectiblesToConvert) {
                const idCount = collectiblesToConvert.filter((id) => id === c).length
                const ownershipCount = collectibleOwnerships.filter((co) => co.collectible_id === c).length
                if (idCount > ownershipCount) {
                    return res.code(400).send({ error: 'You do not own all the collectibles you are trying to convert' })
                }
            }

            // build the collectible object from the collectible ids
            const collectibles = collectiblesToConvert.map((c) => allCollectibles.find((ac) => ac.id === c))
            ...
            if (rarity === 4) {
                return res.code(400).send({ error: 'You cannot convert a collectible with this rarity' })
            }

            if (collectibles.some((c) => c && c.rarity_id !== rarity )) {
                return res.code(400).send({ error: 'You cannot convert collectibles of different rarities' })
            }

            // delete the collectible ownerships
            collectiblesToConvert.forEach((c) => {
                const ownership = collectibleOwnerships.find((co) => co.collectible_id === c)?.id
                collectibleOwnerships.splice(collectibleOwnerships.findIndex((co) => co.collectible_id === c), 1)
                if (!ownership) {
                    return res.code(400).send({ error: 'Invalid collectible data' })
                }
