...
<template>
    ...
    <div v-if="!loading" class="discord-link-wrapper w-full h-[85vh] flex items-center justify-center">
        <div v-if="!code"
            ...
            <div class="flex flex-row justify-center items-center space-x-10 mb-8">
                <div
                    class="ring-4 ring-primary ring-offset-2 ring-offset-base-100 rounded-full p-5 hover:scale-110 transition-transform">
                    <img :src="Asset.HOME" class="w-20 h-20" />
                </div>
                <TablerArrowsDiff class="text-5xl text-secondary animate-pulse" />
                <div
                    class="ring-4 ring-primary ring-offset-2 ring-offset-base-100 rounded-full p-5 hover:scale-110 transition-transform">
                    <IcBaselineDiscord class="text-7xl text-primary" />
                </div>
            </div>

            <button @click="link()"
                class="btn btn-primary mt-5 text-xl px-10 py-4 shadow-md hover:scale-105 transition-transform">
